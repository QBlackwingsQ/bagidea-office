...
const I18N = {
  en: {
    ...
    d_brain_p:"Run each agent on its own model. Claude Code stays the engine — only the brain behind it swaps, and it fails open to Claude, so nothing changes until you opt an agent in. Two kinds: <b>direct</b> Anthropic-compatible (Claude, GLM, DeepSeek, Qwen, MiniMax), and OpenAI-compatible <b>via a built-in zero-dependency proxy</b> (OpenAI, Gemini, OpenRouter, NVIDIA, and your own custom providers) — no LiteLLM or Python to install.",
  ...
  th: {
    ...
    d_brain_p:"ให้ agent แต่ละตัวใช้คนละโมเดล Claude Code ยังเป็น engine — เปลี่ยนแค่สมองข้างหลัง และ fail-open กลับเป็น Claude เสมอ จะไม่มีอะไรเปลี่ยนจนกว่าคุณจะเลือกเปิดให้ agent · มี 2 แบบ: <b>ต่อตรง</b> Anthropic-compatible (Claude, GLM, DeepSeek, Qwen, MiniMax) และ OpenAI-compatible <b>ผ่าน proxy ในตัว</b> (OpenAI, Gemini, OpenRouter, NVIDIA และ custom provider ของคุณเอง) — ไม่ต้องลง LiteLLM หรือ Python",
