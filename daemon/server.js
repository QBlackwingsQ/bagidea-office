...
// Auto-compact + fresh-thread recovery (see maybeRecover). Summarize the visible
// thread with Claude (always-present engine, big context) so continuity survives,
// then re-run the SAME task on a new thread seeded with that brief.
async function autoRecoverOverflow(agent, prompt, opts, oldEntry) {
  const a = (reg.agents && reg.agents[agent]) || {};
  const label = a.provider && a.provider !== "claude"
    ? a.provider + (a.model ? "/" + a.model : "") : "โมเดลที่เลือก";
  broadcast({ type: "chat.message", agent, session: oldEntry.key,
    text: `⚠ ${label} รับ context เต็มไม่ไหว — กำลังสรุปใจความเดิมแล้วเปิด thread ใหม่ให้อัตโนมัติ…` });
  let brief = "";
  try {
    const hist = (oldEntry.log || []).slice(-40)
      .map((l) => `${l.who}: ${String(l.text || "")}`).join("\n").slice(0, 12000);
    if (hist.trim())
      brief = await claudeText(
